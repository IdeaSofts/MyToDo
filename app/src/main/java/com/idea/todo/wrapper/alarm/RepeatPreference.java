...
public class RepeatPreference extends ListPreference {
    ...
    private AlarmInfo.DaysOfWeek mDaysOfWeek = new AlarmInfo.DaysOfWeek(0);
    // New value that will be set if a positive result comes back from the
    // dialog.
    private AlarmInfo.DaysOfWeek mNewDaysOfWeek = new AlarmInfo.DaysOfWeek(0);
    ...
    public void setDaysOfWeek(AlarmInfo.DaysOfWeek dow) {
        mDaysOfWeek.set(dow);
        mNewDaysOfWeek.set(dow);
        setSummary(dow.toString(getContext(), true));
    }

    public AlarmInfo.DaysOfWeek getDaysOfWeek() {
        return mDaysOfWeek;
    }
}
